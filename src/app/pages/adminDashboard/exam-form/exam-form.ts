...
import { IQuestion, QuestionType, DifficultyLevel } from '../../../models/Questions/IQuestions';
  ...
  onSubmit(): void {
    ...
    if (this.examForm.valid && this.selectedQuestions.length > 0) {
      ...
      try {
        ...
        setTimeout(() => {
          console.log('Exam creation simulation completed');
          this.isSubmitting = false;
          this.cdr.markForCheck();
          alert('Exam created successfully! (This is a simulation)');
          this.router.navigate(['/admin/exams']);
        }, 2000);
